...
\item {\bf RQ5:}  Are killed mutants reasonablely similar to actual faults?  That is, is the data above based on meaningful changes to code?
\item {\bf RQ6:}  Do individual mutants, prioritized for ease of examination, allow us to identify classes of faults that
 different tools are good at/bad at, and extend this information to improve tools by addressing identified weaknesses?
\item {\bf RQ7:}  How easy is it to identify low-hanging fruit for improving tools \emph{without} prioritization to remove conceptually redundant mutants?
  \end{itemize}
...
\subsubsection{Qualitative Examination of Mutants (RQ5)}

Because examining all mutants manually is infeasible, we focused primarily on examining mutants in the \emph{difference}.  Because our technique is differential, only mutants detected by at least one tool, but not detected by all tools, inform the comparison of static analysis tools.  The vast majority of the mutants in this set were meaningful.  We do not believe that all of them represent faults; some are harmless but unusual code changes.  A large number of the instances where use of {\tt tx.origin} in place of {\tt msg.sender} was flagged seem to us to be strange but not necessarily incorrect code.  On the other hand, at the risk of false positives, it is not unreasonable for tools to report some of these.   Our estimate is that, ignoring {\tt tx.origin} cases, at least 70\% of the mutants detected by one, but not all, tools, represent real bugs, and the failure to detect is roughly equally due to missing detectors and imprecise analysis.

\subsubsection{Improving Slither {\bf (RQ6 and RQ7)}}
...
For Java, again, the large majority of mutants we inspected where at least one tool detected the mutant and at least one tool did not detect it were definitely meaningful.  In particular, for Java, the large majority of mutants involved either deleted method calls or changes to conditionals (e.g., {\tt == null} to {\tt != null} that would clearly introduce potential null pointer exceptions (NPEs).  While the differences between Solidity tools were often due to different detectors, the Java differences seemed mostly rooted in analysis engine methods; all tools aim to warn about potential NPEs.
...
For Python, as with Java, many mutants we inspected involved deletion of a call, leading to a likely method call or access to a variable containing {\tt None}.  More Python mutants were more difficult to understand than for Solidity or Java, without a grasp of the larger program context, but a large number were still clearly differences in analysis engine capabilities, since all tools aim to detect failure to guard against improper uses of {\tt None}, the Python equivalent of a Java NPE.
...
We used the Universal Mutator \cite{universalmutator,regexpMut}, which aggressively produces large numbers of mutants, but does not target any particular software defect patterns, to generate all mutants.  This tool has been heavily tuned and used for for Solidity and Python, and moderately tuned for Java.  The Java mutation also benefits from extensive tuning on C programs, which share syntactic mutation patterns with both Java and Solidity.
There is no room in the paper to present the exact set of projects analyzed, but we have provided an (anonymized) github repository containing raw results for inspection by reviewers, or further analysis by other researchers.  We will de-anonymize this if the paper is accepted.